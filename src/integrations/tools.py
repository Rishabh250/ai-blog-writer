...
class FetchGoogleTrendsDataTool:
    ...
    def format_trends_for_llm(self, trends_data: Dict) -> Dict:
        ...
        for period_key, period_data in trends_data.get('time_periods', {}).items():
            ...
            for point in timeline_data:
                ...
                for value in values:
                    if value.get('query', '') == trends_data.get('query', '') and int(value.get('extracted_value', 0)) > 50:
                        peak_points.append({
                            'date': date,
    ...
    def _generate_insights(self, formatted_data: Dict) -> List[str]:
        ...
        if formatted_data['interest_peaks']:
            sorted_peaks = sorted(formatted_data['interest_peaks'], key=lambda x: x['value'], reverse=True)
            insights.append(f"Peak interest in '{formatted_data['query']}' occurred on {sorted_peaks[0]['date']}")
            if len(sorted_peaks) > 1:
                insights.append(f"There were {len(sorted_peaks)} significant spikes in interest over the analyzed periods")
        else:
            insights.append(f"Interest in '{formatted_data['query']}' has been relatively steady with no major spikes")
        if formatted_data['related_topics']:
            sorted_topics = sorted(formatted_data['related_topics'], key=lambda x: x['peak_interest'], reverse=True)
            top_related = sorted_topics[0]['keyword']
            insights.append(f"'{top_related}' is a highly related topic with significant interest")
